...
//const { data: products, isError, isLoading } = useGetAllProductsQuery();
  ...
  //const { /*data: products*/ isError, isLoading } = false;
  ...
  useEffect(() => {
    ...
    // this if is to no create an eternal loop, when be using the price filter
    // this function GetAllProduct() gets all products, when there is not any
    if (products?.length===1) { GetAllProduct();}
  
    // when get all the products compare prices to get minPrice and maxPrice values
  ...
  if (/*!isLoading && !isError &&*/ products?.length === 0) {
    content = <ErrorMsg msg="No Products found! cause products.length === 0" />;
  }
  if (/*!isLoading && !isError && */products?.length > 0) {
    // products
  ...
  if (query.color) {
      product_items = products.filter((product) => {
        for (let i = 0; i < product.imgURL.length; i++) {
          const color = product.imgURL[i]?.color;
          if (
            color &&
