...
export const CreateProduct = async (data, selectCategory) => {

    try {
        // create-category with data from form to backend with axios and action POST, data: name, slug
        const res = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}`+'/api/product/add',data)
       ...
       /* alert('res.data.status:'+res.data.status) //- LINE ONLY TO TEST RES DATA
        ...
        if (res.data?.status=='true') {
            notifySuccess(`${res.data?.data.name}, Product created successfully`);
        } //else {notifyError(res.data.message);}
      } catch (error) {
        console.log(error);
        notifyError('Something went wrong in form Product handleSubmit')
      }
  };