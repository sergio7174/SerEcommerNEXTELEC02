...
import { GetAllProductService } from "../../adminDashboard/product/getAllProductService";
...
const ColorFilter = ({setCurrPage,shop_right=false}) => {
  ...
  if (/*!isLoading && !isError && */products?.length > 0) {
    ...
    product_items.forEach((product) => {
      ...
      alert("product.imgURL"+product?.imgURL);

      //let uniqueColor = new Set(product?.imgURL?.map((item) => item?.color));
      let uniqueColor = new Set([product_items]?.imgURL?.map((item) => item?.color));
