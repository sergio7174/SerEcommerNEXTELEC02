import React,{useEffect, useState} from "react";
import Image from "next/image";
import { useRouter } from "next/router";
// internal
import { useGetProductTypeCategoryQuery } from "@/redux/features/categoryApi";
import ErrorMsg from "@/components/common/error-msg";
import Loader from "@/components/loader/loader";

import { GetAllCategoryService } from "@/components/adminDashboard/category/getAllCategoryService";

import Link from "next/link";

const HeaderCategory = ({ isCategoryActive, categoryType = "electronics" }) => {


  const router = useRouter();

    // local const handle with useState hook
    const [categories, setCategories] = useState([]);
  
  //const {data: categories,isError, isLoading} = useGetProductTypeCategoryQuery(categoryType);
  const {isError, isLoading} = useGetProductTypeCategoryQuery(categoryType);
  // get products categories from backend

  //const categories = ()=> {GetAllCategoryService};
  const GetAllCategory =  () => { GetAllCategoryService(setCategories)}

  useEffect(() => {
    // function call GetAllCategory()
    GetAllCategory();
  }, []);


  // handle category route
 /* const handleCategoryRoute = (name, route) => {
    if (route === "parent") {
      router.push(
        `/shop?category=${name
          .toLowerCase()
          .replace("&", "")
          .split(" ")
          .join("-")}`
      );
    } else {
      router.push(
        `/shop?subCategory=${name
          .toLowerCase()
          .replace("&", "")
          .split(" ")
          .join("-")}`
      );
    }
  };*/
  // decide what to render
  let content = null;

  if (isLoading) {
    content = (
      <div className="py-5">
        <Loader loading={isLoading} />
      </div>
    );
  }
  if (!isLoading && isError) {
    content = <ErrorMsg msg="There was an error" />;
  }
  if (!isLoading && !isError && categories?.length === 0) {
    content = <ErrorMsg msg="No Category found!" />;
  }
 // if (!isLoading && !isError && categories?.result?.length > 0) {
  if (!isLoading && !isError && categories?.length > 0) {
    const category_items = categories;
    content = category_items.map((item) => (
      <li className="has-dropdown" key={item._id}>
        <a className="cursor-pointer" style={{textAlign:'center'}}>
          {item.imgURL && (
     <>      
        <span>
           <Link href={{pathname:'/shop-category',query:{name:(item.name)}}}>
             <img src={item.imgURL} alt="product-category" width={76} height={98} />
           </Link>
        </span>
     </>
          )}
       
         {item.name}
        </a>
      </li>
    ));
  }
 
  return <ul className={isCategoryActive ? "active" : ""}>{content}</ul>;
};

export default HeaderCategory;
